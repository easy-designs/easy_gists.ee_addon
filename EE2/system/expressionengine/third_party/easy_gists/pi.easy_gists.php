...
* raw: accepts "yes" or "no" ("no" by default) and has the script load the raw code (as opposed to the parsed script contents)
* wrap_with: the elements you want to wrap around the raw code ("pre.ext,code" by default). Separate multiple elements by a comma. Use .ext to add a class equal to the file’s extension to a specific element.
* highlight: Highlight one or more lines (available in raw mode only). Individual lines should be separated by commas ("10,12"). Multiple lines can be higlighted by setting a start and end, separated by a hyphen ("10-12").
* highlight_with: The markup you want to wrap the highlighted lines ("mark" by default).
